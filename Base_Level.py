...
def main(main_menu):
    config.current_dificulty = "Base"

    # Time limit for the whole game
    config.timer_minutes = config.base_timer_minutes
    config.timer_seconds = config.base_timer_seconds

    base_starting_speed = config.base_starting_speed
    config.current_starting_speed = base_starting_speed
    config.current_speed_timer = base_starting_speed

    # To check that speed doesn't increase faster that the current level allows
    base_max_speed = config.base_max_speed
    config.current_max_speed = base_max_speed
